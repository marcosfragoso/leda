...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
	...
	private void heapify(int position) {
		if (isValidIndex(position)) {
			...
			int largest;
			if (isValidIndex(left) && comparator.compare(this.heap[left], this.heap[position]) > 0) {
				largest = left;
			} else {
				largest = position;
			}
			if (isValidIndex(right) && comparator.compare(this.heap[right], this.heap[largest]) > 0) {
				largest = right;
			}
			if (largest != position) {
	...
	public void insert(T element) {
		...
		while (index > 0 && comparator.compare(this.heap[parent(index)], this.heap[index]) < 0) {
			Util.swap(this.heap, index, parent(index));
			index = parent(index);
		}
	}
	...
	public T rootElement() {
		T resp = null;
		if (!this.isEmpty()) resp = this.heap[0];
		return resp;
	}
